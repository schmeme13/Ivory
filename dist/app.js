...
function options() { return {bpm: el('bpm').value ? Number(el('bpm').value) : null, meter: el('meter').value}; }
...
el('audio').addEventListener('change', () => { if (sourceUrl) URL.revokeObjectURL(sourceUrl); const file = el('audio').files[0]; el('player').hidden = !file; if(file){sourceUrl=URL.createObjectURL(file);el('player').src=sourceUrl;} });
el('upload').addEventListener('submit', async event => {
  ...
  try {
    ...
    const data = new FormData(); data.append('file',file); data.append('signature',options().meter); if(options().bpm) data.append('bpm',options().bpm);
